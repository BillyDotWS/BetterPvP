...
public class SeismicSlam extends Skill implements InteractSkill, CooldownSkill, Listener, OffensiveSkill, MovementSkill, CrowdControlSkill, DamageSkill {
        ...
        while (slamIterator.hasNext()) {
            ...
            if (player != null) {
                ...
                if (timeElapsed && isPlayerGrounded) {
                    slam(player, System.currentTimeMillis() - activationTime);
                    slamIterator.remove();
                }
            } else {
                slamIterator.remove();
    ...
    public void slam(final Player player, final long millisInAir) {
        ...
        new SeismicSlam.LandEvent(targets, player, millisInAir, landLocation).callEvent();
